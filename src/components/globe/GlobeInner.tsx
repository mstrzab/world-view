...
export default function GlobeInner({ visualMode, onViewerReady }: GlobeProps) {
  ...
  const applyVisualEffect = useCallback((mode: string, viewer: Viewer) => {
    try {
      ...
      console.error('Error applying visual effect:', err);
    }
  }, []);
    ...
    return (
      <div className="w-full h-full bg-[#0a0a0f] flex items-center justify-center">
        <div className="text-center p-8">
          <div className="text-red-400 font-mono text-lg mb-4">ERROR</div>
          <div className="text-gray-400 text-sm mb-4">{error}</div>
          <div className="text-gray-500 text-xs">
            WebGL may not be supported or there was an issue loading CesiumJS.
          </div>
        </div>
      </div>
    );
